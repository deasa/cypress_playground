...
describe("Home Page Table Data", function () {
  before(function () {
    cy.loginAsAdmin();
    cy.server();
    cy.fixture("allMBRsNoModules_placeholderMBR.json")
      .as("placeholderMBR")
      .then(function (json) {
  ...
  it("displays data in the correct columns", function () {
    ...
    cy.get(selectors.partDescriptionCell).should(
      "have.text",
      mbr.PartDescription
    );
    cy.get(selectors.productionVersionCell).should(
      "have.text",
      mbr.ProductionVersion
    );
